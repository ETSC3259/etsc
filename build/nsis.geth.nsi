...
# 1. install getsc for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for getsc
# 5. create getsc, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable etsc_SOCKET
...
!define GROUPNAME "etsc"
!define APPNAME "getsc"
!define DESCRIPTION "Official Go implementation of the etsc protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since getsc is installed global
