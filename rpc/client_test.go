	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("EtscSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("EtscSubscribe shouldn't have panicked for %#v", arg)
		...
		client.EtscSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("etsc", new(NotificationTestService))
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.EtscSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while EtscSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	server := newTestServer("etsc", service)
	defer server.Stop()
	client := DialInProc(server)
	defer client.Close()
	...
	go func() {
		sub, err = client.EtscSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("EtscSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("EtscSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("EtscSubscribe did not return within 1s after Close")
	}
}

// This test reproduces https://github.com/ETSC3259/etsc/issues/17837 where the
...
func TestClientCloseUnsubscribeRace(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		client := DialInProc(server)
		nc := make(chan int)
		sub, err := client.EtscSubscribe(context.Background(), nc, "someSubscription", 3, 1)
		if err != nil {
			t.Fatal(err)
		}
		go client.Close()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.EtscSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			var r int
			err := client.CallContext(ctx, &r, "etsc_echo", i)
			if err != nil {
				if !wantError {
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-etsc-test-ipc-%d-%d", os.Getpid(), rand.Int63())
