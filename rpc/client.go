...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout etsc_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for EtscSubscribe requests
}
...
// EtscSubscribe registers a subscripion under the "etsc" namespace.
func (c *Client) EtscSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "etsc", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. EtscSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through EtscSubscribe.
