...
import (
	...
	"github.com/ETSC3259/etsc/internal/jsre"
	"github.com/ETSC3259/etsc/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute etsc requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jetsc", struct{}{})

	jetscObj, _ := c.jsre.Get("jetsc")
	jetscObj.Object().Set("send", bridge.Send)
	jetscObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jetsc);"); err != nil {
	...
	flatten := "var etsc = web3.etsc; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = etsc.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jetsc.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jetsc.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jetsc.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jetsc.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jetsc.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines etsc.getBalance(etsc.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current getsc instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic getsc metadata
	fmt.Fprintf(c.printer, "Welcome to the getsc JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + etsc.coinbase);
		console.log("at block: " + etsc.blockNumber + " (" + new Date(1000 * etsc.getBlock(etsc.blockNumber).timestamp) + ")");
