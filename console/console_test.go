...
func newTester(t *testing.T, confOverride func(*etsc.Config)) *tester {
	...
	// Create a networkless protocol stack and start an etsc service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	etscConf := &etsc.Config{
		Genesis:   core.DeveloperGenesisBlock(15, common.Address{}),
		Etscbase: common.HexToAddress(testAddress),
		Etschash: etschash.Config{
			PowMode: etschash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(etscConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return etsc.New(ctx, etscConf) }); err != nil {
		t.Fatalf("failed to register etsc protocol: %v", err)
	}
