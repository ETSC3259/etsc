...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", getsc.LightScryptN, getsc.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.etsc", new NodeConfig());
			node.start();
			...
			etscClient ec = node.getetscClient();
	...
	// signatures too. Regression test for go-etsc issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "getsc-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/etsc/getsctest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for getsc and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.etsc", "github.com/ETSC3259/etsc/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "getsc.aar"), "getsc.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "etsc", "getsctest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
