...
// getscex retrieves the hex string representation of the block nonce.
func (n *Nonce) getscex() string {
...
// getscex retrieves the hex string representation of the bloom filter.
func (b *Bloom) getscex() string {
	return fmt.Sprintf("0x%x", b.bloom[:])
}

// Header represents a block header in the etsc blockchain.
type Header struct {
	header *types.Header
}
...
func (h *Header) getscash() *Hash         { return &Hash{h.header.Hash()} }
...
// Block represents an entire block in the etsc blockchain.
type Block struct {
	block *types.Block
}
...
func (b *Block) getscash() *Hash                 { return &Hash{b.block.Hash()} }
func (b *Block) getsceader() *Header             { return &Header{b.block.Header()} }
...
// Transaction represents a single etsc transaction.
...
func (tx *Transaction) getscash() *Hash   { return &Hash{tx.tx.Hash()} }
...
// Deprecated: use etscClient.TransactionSender
...
func (m *Message) getscash() []byte     { return m.message.Hash }
