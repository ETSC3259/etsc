...
// faucet is a Etsc faucet backed by a light client.
package main
...
var (
	...
	ethPortFlag = flag.Int("etscport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the etsc protocol")
	statsFlag   = flag.String("etscstats", "", "Etscstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Etscs to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for etsc and the faucet")
)

var (
	etsc = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Etscs", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *etscPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // etsc address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an etsc light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // etsc protocol stack
	client *etscclient.Client   // Client connection to the etsc chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the etsc light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := etsc.DefaultConfig
	...
	// Assemble the etscstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lightetsc
			ctx.Service(&serv)
			return etscstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := etscclient.NewClient(api)
...
// close terminates the etsc connection and tears down the faucet.
...
// apiHandler handles requests for Etsc grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    new(big.Int).Div(balance, etsc),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the etsc address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/ETSC3259/etsc/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), etsc)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance := new(big.Int).Div(f.balance, etsc)
			peers := f.stack.Server().PeerCount()
...
// the username, avatar URL and etsc address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the etsc address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No etsc address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and etsc address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain etsc address,
