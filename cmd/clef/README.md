Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for getsc's account management.
This allows DApps not to depend on getsc's account management. When a DApp wants to sign data it can send the data to
the signer, the signer will then provide the user with context and asks the user for permission to sign the data. If
the users grants the signing request the signer will send the signature back to the DApp.
  
This setup allows a DApp to connect to a remote etsc node and send transactions that are locally signed. This can
help in situations when a DApp is connected to a remote node because a local etsc node is not available, not
synchronised with the chain or a particular etsc node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.etsc/keystore")
...
The general flow for signing a transaction using e.g. getsc is as follows:
![image](sign_flow.png)

In this case, `getsc` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `etsc.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: getsc should be started in `getsc --external_signer localhost:8550`.
    - Currently, the getsc APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The getsc api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus getsc/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as getsc. The messages are
...
The signer will generate a new private key, encrypts it according to [web3 keystore spec](https://github.com/etsc/wiki/wiki/Web3-Secret-Storage-Definition) and stores it in the keystore directory.
...
#### Arguments
  - account [object]: key in [web3 keystore format](https://github.com/etsc/wiki/wiki/Web3-Secret-Storage-Definition) (retrieved with account_export)
...
#### Result
  - exported key, see [web3 keystore format](https://github.com/etsc/wiki/wiki/Web3-Secret-Storage-Definition) for
  more information
  ...
  "params": [
    {
      "accounts": [
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.etsc/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.etsc/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for etsc addresses.
