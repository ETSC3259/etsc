...
```text
#cp $GOPATH/src/github.com/ETSC3259/etsc/cmd/clef/4byte.json $PWD
```
...
_Note: make sure that `0x694...` is an account you have access to. You can create it either via the clef or the traditional account cli tool. If the latter was chosen, make sure both clef and getsc use the same keystore by specifing `--keystore path/to/your/keystore` when running clef._
