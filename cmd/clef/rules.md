...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 etsc` in value to maximum `1 etsc` per 24h period
* I want to auto-approve transaction to contract `EtscAlarmClock` with `data`=`0xdeadbeef`, if `value=0`, `gas < 44k` and `gasPrice < 40Gwei`
...
implemented for `getsc`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
