...
func TestAccountListEmpty(t *testing.T) {
	getsc := rungetsc(t, "account", "list")
	getsc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	getsc := rungetsc(t, "account", "list", "--datadir", datadir)
	defer getsc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	getsc := rungetsc(t, "account", "new", "--lightkdf")
	defer getsc.ExpectExit()
	getsc.Expect(`
...
`)
	getsc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	getsc := rungetsc(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	getsc := rungetsc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer getsc.ExpectExit()
	getsc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(getsc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(getsc.StderrText(), m) {
...
// https://github.com/ETSC3259/etsc/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	getsc.SetTemplateFunc("keypath", func(file string) string {
