...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.etscstats == "" {
		log.Error("No etscstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Etschash != nil && !boot {
		fmt.Println()
		if infos.etschashdir == "" {
			fmt.Printf("Where should the etschash mining DAGs be stored on the remote machine?\n")
			infos.etschashdir = w.readString()
		} else {
			fmt.Printf("Where should the etschash mining DAGs be stored on the remote machine? (default = %s)\n", infos.etschashdir)
			infos.etschashdir = w.readDefaultString(infos.etschashdir)
		}
	}
	...
	if infos.etscstats == "" {
		...
		infos.etscstats = w.readString() + ":" + w.conf.etscstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.etscstats)
		infos.etscstats = w.readDefaultString(infos.etscstats) + ":" + w.conf.etscstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Etschash != nil {
			// Etschash based miners only need an etscbase to mine against
			fmt.Println()
			if infos.etscbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.etscbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.etscbase)
				infos.etscbase = w.readDefaultAddress(common.HexToAddress(infos.etscbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy etsc node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
