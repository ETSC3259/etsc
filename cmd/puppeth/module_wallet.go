...
FROM puppetsc/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'getsc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec getsc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --etscstats \'{{.Etscstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/puppetscNetworkID/{{.NetworkID}}/g' dist/js/etscwallet-master.js && \
	sed -i 's/puppetscNetwork/{{.Network}}/g'     dist/js/etscwallet-master.js && \
	sed -i 's/puppetscDenom/{{.Denom}}/g'         dist/js/etscwallet-master.js && \
	sed -i 's/puppetscHost/{{.Host}}/g'           dist/js/etscwallet-master.js && \
	sed -i 's/puppetscRPCPort/{{.RPCPort}}/g'     dist/js/etscwallet-master.js
