...
// makeWizard creates and returns a new puppetsc wizard.
...
// setting up a new or managing an existing etsc private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppetsc, your etsc private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new etsc network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and etscstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| puppetsc uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering etsc network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppetsc", w.network)
