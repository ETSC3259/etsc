...
// nodeDockerfile is the Dockerfile required to run an etsc node.
var nodeDockerfile = `
FROM etsc/client-go:latest
...
{{end}}
RUN \
  echo 'getsc --cache 512 init /genesis.json' > getsc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.etsc/keystore/ && cp /signer.json /root/.etsc/keystore/' >> getsc.sh && \{{end}}
	echo $'exec getsc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --etscstats \'{{.Etscstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etscbase}}--miner.etscbase {{.Etscbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> getsc.sh

ENTRYPOINT ["/bin/sh", "getsc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an etsc node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.etsc{{if .Etschashdir}}
      - {{.Etschashdir}}:/root/.etschash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Etscstats}}
      - MINER_NAME={{.Etscbase}}
...
// deployNode deploys a new etsc node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.etscbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Etschashdir":  config.etschashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Etscstats":   config.etscstats[:strings.Index(config.etscstats, ":")],
		"Etscbase":  config.etscbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Etscstats username":        info.etscstats,
	}
	if info.gasTarget > 0 {
		...
		if info.etscbase != "" {
			// Etschash proof-of-work miner
			report["Etschash directory"] = info.etschashdir
			report["Miner account"] = info.etscbase
		}
		if info.keyJSON != "" {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 getsc --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.etsc"],
		etschashdir:  infos.volumes["/root/.etschash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		etscstats:   infos.envvars["STATS_NAME"],
		etscbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
