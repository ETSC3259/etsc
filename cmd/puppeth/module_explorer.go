...
FROM puppetsc/explorer:latest

ADD etscstats.json /etscstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../etsc-net-intelligence-api && pm2 start /etscstats.json)' >  explorer.sh && \
	echo '(cd ../etscchain-light && npm start &)'                      >> explorer.sh && \
	echo 'exec /parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEtscstats is the configuration file for the etscstats javascript client.
var explorerEtscstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.etsc
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Etscstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	etscstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEtscstats)).Execute(etscstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.etscstats[:strings.Index(config.etscstats, ":")],
		"Secret": config.etscstats[strings.Index(config.etscstats, ":")+1 : strings.Index(config.etscstats, "@")],
		"Host":   config.etscstats[strings.Index(config.etscstats, "@")+1:],
	})
	files[filepath.Join(workdir, "etscstats.json")] = etscstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.etsc"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		etscstats: infos.envvars["STATS"],
	}
	return stats, nil
}
