...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Etscs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.etscstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.etscstats = w.readString() + ":" + w.conf.etscstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.etscstats)
		infos.node.etscstats = w.readDefaultString(infos.node.etscstats) + ":" + w.conf.etscstats
	}
