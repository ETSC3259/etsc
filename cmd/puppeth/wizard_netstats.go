...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for etscstats availability")
	if infos, err := checkEtscstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["etscstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["etscstats"] = infos.Report()
		etscstats = infos.config
	}
