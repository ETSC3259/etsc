...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"etscstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "etscstats":
				if infos, err := checkEtscstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have etscstats running, ask whether to make the secret public or not
	if w.conf.etscstats != "" {
		fmt.Println()
		fmt.Println("Include etscstats secret on dashboard (y/n)? (default = yes)")
