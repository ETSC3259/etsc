...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom etschash engine
	etschash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	...
	etschash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := etschash.SealHash(header).Hex(); work[0] != want {
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			etschash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, etschash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch etschash result timeout", id+1)
		}
	}
}
