...
type Vote struct {
	...
	Authorize bool           `json:"authorize"` // Whetsc to authorize or deauthorize the voted account
}

// Tally is a simple vote tally to keep the current score of votes. Votes that
// go against the proposal aren't counted since it's equivalent to not voting.
type Tally struct {
	Authorize bool `json:"authorize"` // Whetsc the vote is about authorizing or kicking someone
...
func loadSnapshot(config *params.CliqueConfig, sigcache *lru.ARCCache, db etscdb.Database, hash common.Hash) (*Snapshot, error) {
...
func (s *Snapshot) store(db etscdb.Database) error {
