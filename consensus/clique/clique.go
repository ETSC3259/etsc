...
// ecrecover extracts the etsc account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the etsc address
...
// etsc testnet following the Ropsten attacks.
type Clique struct {
	config *params.CliqueConfig // Consensus engine configuration parameters
	db     etscdb.Database       // Database to store and retrieve snapshot checkpoints
	...
	signer common.Address // etsc address of the signing key
...
func New(config *params.CliqueConfig, db etscdb.Database) *Clique {
...
// Author implements consensus.Engine, returning the etsc address recovered
...
func (c *Clique) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 || (number%c.config.Epoch == 0 && chain.getsceaderByNumber(number-1) == nil) {
			checkpoint := chain.getsceaderByNumber(number)
			if checkpoint != nil {
		...
		if len(parents) > 0 {
			...
			header = chain.getsceader(hash, number)
			if header == nil {
				return nil, consensus.ErrUnknownAncestor
			}
		}
