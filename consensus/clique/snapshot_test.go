...
// mapped from textual names used in the tests below to actual etsc private
...
// address retrieves the etsc address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the etsc address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/ETSC3259/etsc/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// getsc the header and prepare it for signing
			header := block.Header()
			if j > 0 {
