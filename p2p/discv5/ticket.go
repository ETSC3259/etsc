...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, targetscash common.Hash, t *ticket) {
	for i, topic := range t.topics {
		if tt, ok := s.radius[topic]; ok {
			tt.adjustWithTicket(now, targetscash, ticketRef{t, i})
		}
	}
}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, targetscash common.Hash, t ticketRef) {
	...
	r.adjust(now, targetscash, t.t.node.sha, inside)
}

func (r *topicRadius) adjust(now mclock.AbsTime, targetscash, addrHash common.Hash, inside float64) {
	...
	delete(r.buckets[bucket].lookupSent, targetscash)
}
