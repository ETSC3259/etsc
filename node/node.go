...
func (n *Node) OpenDatabase(name string, cache, handles int) (etscdb.Database, error) {
	if n.config.DataDir == "" {
		return etscdb.NewMemDatabase(), nil
	}
	return etscdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
