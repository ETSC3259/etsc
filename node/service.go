...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (etscdb.Database, error) {
	if ctx.config.DataDir == "" {
		return etscdb.NewMemDatabase(), nil
	}
	db, err := etscdb.NewLDBDatabase(ctx.config.ResolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
