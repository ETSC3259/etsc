...
[https://swarm.etsccoin.com](https://swarm.etsccoin.com)

Swarm is a distributed storage platform and content distribution service, a native base layer service of the etsc web3 stack. The primary objective of Swarm is to provide a decentralized and redundant store for dapp code and data as well as block chain and state data. Swarm is also set out to provide various base layer services for web3, including node-to-node messaging, media streaming, decentralised database services and scalable state-channel infrastructure for decentralised service economies.

[![Travis](https://travis-ci.org/ETSC3259/etsc.svg?branch=master)](https://travis-ci.org/ETSC3259/etsc)
[![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/etscsphere/orange-lounge?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/ETSC3259/etsc

    go install github.com/ETSC3259/etsc/cmd/swarm
...
To run Swarm you need an etsc account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the etsc address representing that account. For example:
...
### etsc Name Service resolution

The etsc Name Service is the etsc equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.etsc`), `swarm` has to connect to a `getsc` instance, which is synced with the etsc mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.etsc/getsc.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/ETSC3259/etsc`.

Most likely you will be working from your fork of `go-etsc`, let's say from `github.com/nirname/go-etsc`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-etsc.git $GOPATH/src/github.com/ETSC3259/etsc
```
...
```
go test -v -cpu 4 ./etsc -run TestMethod
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/ETSC3259/etsc/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The etsc Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
...
You can find a few reference Swarm decentralised applications at: https://swarm-gateways.net/bzz:/swarmapps.etsc

Their source code can be found at: https://github.com/etscsphere/swarm-dapps
...
complex changes though, please check up with the core devs first on [our Swarm gitter channel](https://gitter.im/etscsphere/orange-lounge)
 ...
 * [Code review guidelines](https://github.com/ETSC3259/etsc/wiki/Code-Review-Guidelines).
...
The go-etsc library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html), also
included in our repository in the `COPYING.LESSER` file.

The go-etsc binaries (i.e. all code inside of the `cmd` directory) is licensed under the
