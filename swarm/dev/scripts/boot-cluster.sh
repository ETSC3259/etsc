...
#
# The cluster contains a bootnode, a getsc node and multiple swarm nodes, with
...
# static getsc configuration
getsc_IP="192.168.33.3"
getsc_RPC_PORT="8545"
getsc_RPC_URL="http://${getsc_IP}:${getsc_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the getsc and
# swarm nodes
start_bootnode() {
...
# start_getsc_node starts a getsc node with --datadir pointing at <base-dir>/getsc
# and a single, unlocked account with password "getsc"
start_getsc_node() {
  local dir="${base_dir}/getsc"
  mkdir -p "${dir}"

  local password="getsc"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating getsc account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get getsc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${getsc_IP}"
    --rpcport   "${getsc_RPC_PORT}"
    --verbosity "6"
  )

  start_node "getsc" "${getsc_IP}" "$(which getsc)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vetsc pair
  local vetsc0="vetsc${name}0"
  local vetsc1="vetsc${name}1"
  ip link add name "${vetsc0}" type vetsc peer name "${vetsc1}"

  # add one end to the bridge
  ip link set dev "${vetsc0}" master "${BRIDGE_NAME}"
  ip link set dev "${vetsc0}" up

  # add the other end to the namespace, rename it etsc0 and give it the ip
  ip link set dev "${vetsc1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vetsc1}" name "etsc0"
  ip netns exec "${name}" ip link set dev "etsc0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "etsc0"
}
...
create_account() {
  ...
  getsc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
