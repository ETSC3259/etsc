...
func testHandshake(t *testing.T) {
	...
	err = clients[0].Call(&lsendsymkeyids, "pss_getscandshakeKeys", rpubkey, topic, false, true)
	if err != nil {
		t.Fatal(err)
	}
	m := 0
	...
	err = clients[1].Call(&rsendsymkeyids, "pss_getscandshakeKeys", lpubkey, topic, false, true)
	if err != nil {
		t.Fatal(err)
	}
	var lrecvsymkeyids []string
	err = clients[0].Call(&lrecvsymkeyids, "pss_getscandshakeKeys", rpubkey, topic, true, false)
	if err != nil {
		t.Fatal(err)
	}
	var rrecvsymkeyids []string
	err = clients[1].Call(&rrecvsymkeyids, "pss_getscandshakeKeys", lpubkey, topic, true, false)
	if err != nil {
		t.Fatal(err)
	}
