...
func (rw *pssRPCRW) WriteMsg(msg p2p.Msg) error {
	...
	err = rw.Client.rpc.Call(&symkeyids, "pss_getscandshakeKeys", rw.pubKeyId, rw.topic, false, true)
	if err != nil {
		return err
	}
	...
	if len(symkeyids) > 0 {
		err = rw.Client.rpc.Call(&symkeycap, "pss_getscandshakeKeyCapacity", symkeyids[0])
		if err != nil {
			return err
		}
	}
	...
	go func() {
		for {
			...
			case msg := <-msgC:
				...
				var pubkeyid string
				err = c.rpc.Call(&pubkeyid, "pss_getscandshakePublicKey", msg.Key)
				if err != nil || pubkeyid == "" {
