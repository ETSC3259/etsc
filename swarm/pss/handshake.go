...
func (api *HandshakeAPI) getscandshakeKeys(pubkeyid string, topic Topic, in bool, out bool) (keys []string, err error) {
	if in {
...
func (api *HandshakeAPI) getscandshakeKeyCapacity(symkeyid string) (uint16, error) {
...
func (api *HandshakeAPI) getscandshakePublicKey(symkeyid string) (string, error) {
	storekey := api.ctrl.symKeyIndex[symkeyid]
	if storekey == nil {
