...
import (
	...
	getscmetrics "github.com/ETSC3259/etsc/metrics"
...
func Setup(ctx *cli.Context) {
	if getscmetrics.Enabled {
		...
		go getscmetrics.CollectProcessMetrics(2 * time.Second)

		if enableExport {
			log.Info("Enabling swarm metrics export to InfluxDB")
			go influxdb.InfluxDBWithTags(getscmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
