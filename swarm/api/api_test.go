...
import (
	...
	"github.com/ETSC3259/etsc/swarm/sctx"
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.etsc"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.etsc"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.etsc"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(ethResolve, "etsc"),
			),
			addr:   ethAddr,
			result: ethHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "etsc"),
				MultiResolverOptionWithResolver(doesntResolve, "etsc"),
				MultiResolverOptionWithResolver(ethResolve, "etsc"),
			),
			addr:   ethAddr,
		...
		},
		{
			...
			addr:   ethAddr,
			result: ethHash,
			err:    errors.New(`DNS name not found: "swarm.etsc"`),
		},
		{
