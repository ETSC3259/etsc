...
import (
	...
	"github.com/ETSC3259/etsc/swarm/network/stream/intervals"
	"github.com/ETSC3259/etsc/swarm/pot"
...
func (r *Registry) Subscribe(peerId enode.ID, s Stream, h *Range, priority uint8) error {
	...
	if s.Live && h != nil {
		if err := peer.setClientParams(
			getscistoryStream(s),
			newClientParams(getscistoryPriority(priority), h.To),
		); err != nil {
			return err
		}
	}
...
func (r *Registry) updateSyncing() {
	...
	kad.EachBin(r.addr[:], pot.DefaultPof(256), 0, func(p *network.Peer, bin int) bool {
		...
		if streams, ok := subs[p.ID()]; ok {
			...
			delete(streams, getscistoryStream(stream))
		}
...
func getscistoryPriority(priority uint8) uint8 {
	if priority == 0 {
		return 0
	}
	return priority - 1
...
func getscistoryStream(s Stream) Stream {
