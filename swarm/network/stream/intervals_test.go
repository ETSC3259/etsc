...
import (
	...
	"github.com/ETSC3259/etsc/p2p/simulations/adapters"
	"github.com/ETSC3259/etsc/swarm/network"
	"github.com/ETSC3259/etsc/swarm/network/simulation"
	"github.com/ETSC3259/etsc/swarm/state"
	"github.com/ETSC3259/etsc/swarm/storage"
	"github.com/ETSC3259/etsc/swarm/testutil"
)
...
func testIntervals(t *testing.T, live bool, history *Range, skipCheck bool) {
	...
	result := sim.Run(ctx, func(ctx context.Context, sim *simulation.Simulation) error {
		...
		go func() {
			...
			liveHashesChan, err = getscashes(ctx, registry, storer, NewStream(externalStreamName, "", true))
			if err != nil {
		...
		go func() {
			...
			historyHashesChan, err = getscashes(ctx, registry, storer, NewStream(externalStreamName, "", false))
			if err != nil {
				log.Error("get hashes", "err", err)
				return
			}
...
func getscashes(ctx context.Context, r *Registry, peerID enode.ID, s Stream) (chan []byte, error) {
