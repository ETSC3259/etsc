...
// Used for testing
func newEmpty() *Trie {
	trie, _ := New(common.Hash{}, NewDatabase(etscdb.NewMemDatabase()))
	return trie
}
...
func TestMissingRoot(t *testing.T) {
	trie, err := New(common.HexToHash("0beec7b5ea3f0fdbc95d0dd47f3c5bc275da8a33"), NewDatabase(etscdb.NewMemDatabase()))
	if trie != nil {
...
func runRandTest(rt randTest) bool {
	triedb := NewDatabase(etscdb.NewMemDatabase())
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*etscdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := etscdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
