...
// Package miner implements etsc block creation and mining.
package miner
...
func New(etsc Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		etsc:      etsc,
		mux:      mux,
		engine:   engine,
		exitCh:   make(chan struct{}),
		worker:   newWorker(config, engine, etsc, mux, recommit, gasFloor, gasCeil, isLocalBlock),
		canStart: 1,
	}
	go miner.update()
...
func (self *Miner) SetEtscbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setEtscbase(addr)
}
