...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the etsc
...
type wallet struct {
	...
	deriveChain    etsc.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived etsc account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain etsc.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
