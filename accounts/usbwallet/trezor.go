...
import (
	...
	"github.com/ETSC3259/etsc/accounts/usbwallet/internal/trezor"
...
// or whether the etsc app was not started on it.
...
// etsc address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.etscAddress)
	if _, err := w.trezorExchange(&trezor.etscGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.etscTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.etscTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
