...
import "github.com/ETSC3259/etsc/accounts/abi"
...
type tmplMethod struct {
	...
	Structured bool       // Whetsc the returns should be accumulated into a struct
}
...
import (
	...
	etsc "github.com/ETSC3259/etsc"
	"github.com/ETSC3259/etsc/accounts/abi"
	"github.com/ETSC3259/etsc/accounts/abi/bind"
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new etsc contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an etsc contract.
	type {{.Type}} struct {
	...
	// {{.Type}}Caller is an auto generated read-only Go binding around an etsc contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an etsc contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Filterer is an auto generated log filtering Go binding around an etsc contract events.
	type {{.Type}}Filterer struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an etsc contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an etsc contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an etsc contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an etsc contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an etsc contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an etsc contract.
	...
	{{range .Events}}
		...
		type {{$contract.Type}}{{.Normalized.Name}}Iterator struct {
			...
			sub  etsc.Subscription // Subscription for errors, completion and termination
			done bool                  // Whetsc the subscription completed delivering logs
...
import org.etsc.getsc.*;
import org.etsc.getsc.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new etsc contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, etscClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = getsc.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, getsc.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(getsc.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// etsc address where this contract is located at.
		public final Address Address;

		// etsc transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, etscClient client) throws Exception {
			this(getsc.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = getsc.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, getsc.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = getsc.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = getsc.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = getsc.newCallOpts();
				}
