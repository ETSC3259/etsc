...
The dashboard is a data visualizer integrated into getsc, intended to collect and visualize useful information of an etsc node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-etsc` repo), these need to be installed first:

```
$ (cd dashboard/assets && yarn install && yarn flow)
```

Normally the dashboard assets are bundled into getsc via `go-bindata` to avoid external dependencies. Rebuilding getsc after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ getsc --dashboard --vmodule=dashboard=5
$ (cd dashboard/assets && yarn dev)
```

To bundle up the final UI into getsc, run `go generate`:
