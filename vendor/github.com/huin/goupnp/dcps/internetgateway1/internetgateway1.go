...
const (
	...
	URN_WANEtscnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANEtscnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Etscnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Etscnet
//
...
// WANEtscnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANEtscnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANEtscnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANEtscnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANEtscnetLinkConfig1Clients() (clients []*WANEtscnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANEtscnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANEtscnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANEtscnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANEtscnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANEtscnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANEtscnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANEtscnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANEtscnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANEtscnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANEtscnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANEtscnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANEtscnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANEtscnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANEtscnetLinkConfig1 {
	clients := make([]*WANEtscnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANEtscnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewEtscnetLinkStatus: allowed values: Up, Down
func (client *WANEtscnetLinkConfig1) GetEtscnetLinkStatus() (NewEtscnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewEtscnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANEtscnetLinkConfig_1, "GetEtscnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewEtscnetLinkStatus, err = soap.UnmarshalString(response.NewEtscnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
