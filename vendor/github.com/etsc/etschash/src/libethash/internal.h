...
#define MIX_WORDS (ETSCHASH_MIX_BYTES/4)
...
static inline uint8_t etschash_h256_get(etschash_h256_t const* hash, unsigned int i)
{
	return hash->b[i];
}

static inline void etschash_h256_set(etschash_h256_t* hash, unsigned int i, uint8_t v)
{
	hash->b[i] = v;
}

static inline void etschash_h256_reset(etschash_h256_t* hash)
{
	memset(hash, 0, 32);
}

// Returns if hash is less than or equal to boundary (2^256/difficulty)
static inline bool etschash_check_difficulty(
	etschash_h256_t const* hash,
	etschash_h256_t const* boundary
)
{
	// Boundary is big endian
	for (int i = 0; i < 32; i++) {
		if (etschash_h256_get(hash, i) == etschash_h256_get(boundary, i)) {
			continue;
		}
		return etschash_h256_get(hash, i) < etschash_h256_get(boundary, i);
	}
	return true;
}
...
/**
 * Allocate and initialize a new etschash_light handler. Internal version
 *
...
/**
 * Allocate and initialize a new etschash_full handler. Internal version.
 *
 * @param dirname        The directory in which to put the DAG file.
 * @param seedhash       The seed hash of the block. Used in the DAG file naming.
 * @param full_size      The size of the full data in bytes.
 * @param cache          A cache object to use that was allocated with @ref etschash_cache_new().
 *                       Iff this function succeeds the etschash_full_t will take memory
 ...
 *
 * @param mem         A pointer to an etschash full's memory
 * @param full_size   The size of the full data in bytes
 * @param cache       A cache object to use in the calculation
 * @param callback    The callback function. Check @ref etschash_full_new() for details.
