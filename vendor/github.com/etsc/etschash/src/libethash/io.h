...
// Reference: https://github.com/etsc/wiki/wiki/Etschash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see etschash_io_prepare
enum etschash_io_rc {
	ETSCHASH_IO_FAIL = 0,           ///< There has been an IO failure
	ETSCHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	ETSCHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	ETSCHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of etschash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * etschash_full_t
 */
#ifdef ETSCHASH_PRINT_CRITICAL_OUTPUT
#define ETSCHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("ETSCHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for etschash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the etschash
 ...
 *                           https://github.com/etsc/wiki/wiki/Etschash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum etschash_io_rc
 */
enum etschash_io_rc etschash_io_prepare(
	char const* dirname,
	etschash_h256_t const seedhash,
	FILE** output_file,
	uint64_t file_size,
	bool force_create
);
 ...
 */
FILE* etschash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* etschash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool etschash_mkdir(char const* dirname);
 ...
 */
bool etschash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* etschash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/etsc/wiki/wiki/Etschash-DAG
 *
 ...
 */
bool etschash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool etschash_io_mutable_name(
	uint32_t revision,
