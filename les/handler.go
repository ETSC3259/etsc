...
// Package les implements the Light etsc Subprotocol.
package les
...
import (
	...
	"github.com/ETSC3259/etsc/common/mclock"
...
const (
	...
	ethVersion = 63 // equivalent etsc version for the downloader
...
type BlockChain interface {
	...
	getsceader(hash common.Hash, number uint64) *types.Header
	getsceaderByHash(hash common.Hash) *types.Header
	CurrentHeader() *types.Header
	GetTd(hash common.Hash, number uint64) *big.Int
	State() (*state.StateDB, error)
	InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error)
	Rollback(chain []common.Hash)
	getsceaderByNumber(number uint64) *types.Header
...
// NewProtocolManager returns a new etsc sub protocol manager. The etsc sub protocol manages peers capable
// with the etsc network.
func NewProtocolManager(chainConfig *params.ChainConfig, indexerConfig *light.IndexerConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb etscdb.Database, odr *LesOdr, txrelay *LesTxRelay, serverPool *serverPool, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light etsc protocol")
	...
	log.Info("Light etsc protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light etsc peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light etsc handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light etsc peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light etsc message handling failed", "err", err)
			return err
		}
	}
}

var reqList = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, GetProofsV1Msg, SendTxMsg, SendTxV2Msg, GetTxStatusMsg, getsceaderProofsMsg, GetProofsV2Msg, getscelperTrieProofsMsg}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light etsc message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit {
			...
			if hashMode {
				if first {
					first = false
					origin = pm.blockchain.getsceaderByHash(query.Origin.Hash)
					if origin != nil {
						query.Origin.Number = origin.Number.Uint64()
					}
				} else {
					origin = pm.blockchain.getsceader(query.Origin.Hash, query.Origin.Number)
				}
			} else {
				origin = pm.blockchain.getsceaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case hashMode && !query.Reverse:
				...
				if next <= current {
					...
					if header := pm.blockchain.getsceaderByNumber(next); header != nil {
	...
	case GetReceiptsMsg:
		...
		for _, hash := range req.Hashes {
			...
			if results == nil {
				if header := pm.blockchain.getsceaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					continue
				}
			}
	...
	case getsceaderProofsMsg:
		...
		trieDb := trie.NewDatabase(etscdb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
			if header := pm.blockchain.getsceaderByNumber(req.BlockNum); header != nil {
	...
	case getscelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				var prefix string
				if root, prefix = pm.getscelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(etscdb.NewTable(pm.chainDb, prefix)))
				}
			}
			if req.AuxReq == auxRoot {
				...
				if req.AuxReq != 0 {
					data := pm.getscelperTrieAuxData(req)
					auxData = append(auxData, data)
					auxBytes += len(data)
				}
			}
...
// getscelperTrie returns the post-processed trie root for the given trie ID and section index
func (pm *ProtocolManager) getscelperTrie(id uint, idx uint64) (common.Hash, string) {
	switch id {
	case htCanonical:
...
// getscelperTrieAuxData returns requested auxiliary data for the given HelperTrie request
func (pm *ProtocolManager) getscelperTrieAuxData(req HelperTrieReq) []byte {
