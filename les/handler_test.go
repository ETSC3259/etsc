...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		for _, hash := range tt.expect {
			headers = append(headers, bc.getsceaderByHash(hash))
		}
...
func testGetCode(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.getsceaderByNumber(i)
		req := &CodeReq{
			BHash:  header.Hash(),
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	header := bc.getsceaderByNumber(frequency - 1)
	...
	switch protocol {
	case 1:
		root := light.GetChtRoot(server.db, 0, bc.getsceaderByNumber(frequency-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(etscdb.NewTable(server.db, light.ChtTablePrefix)))
	...
	case 2:
		root := light.GetChtRoot(server.db, (frequency/config.ChtSize)-1, bc.getsceaderByNumber(frequency-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(etscdb.NewTable(server.db, light.ChtTablePrefix)))
	...
	switch protocol {
	case 1:
		cost := server.tPeer.GetRequestCost(getsceaderProofsMsg, len(requestsV1))
		sendRequest(server.tPeer.app, getsceaderProofsMsg, 42, cost, requestsV1)
		if err := expectResponse(server.tPeer.app, HeaderProofsMsg, 42, testBufLimit, proofsV1); err != nil {
			t.Errorf("proofs mismatch: %v", err)
		}
	case 2:
		cost := server.tPeer.GetRequestCost(getscelperTrieProofsMsg, len(requestsV2))
		sendRequest(server.tPeer.app, getscelperTrieProofsMsg, 42, cost, requestsV2)
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		root := light.GetBloomTrieRoot(server.db, 0, bc.getsceaderByNumber(config.BloomTrieSize-1).Hash())
		trie, _ := trie.New(root, trie.NewDatabase(etscdb.NewTable(server.db, light.BloomTrieTablePrefix)))
		trie.Prove(key, 0, &proofs.Proofs)

		// Send the proof request and verify the response
		cost := server.tPeer.GetRequestCost(getscelperTrieProofsMsg, len(requests))
		sendRequest(server.tPeer.app, getscelperTrieProofsMsg, 42, cost, requests)
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), etschash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), etschash.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
