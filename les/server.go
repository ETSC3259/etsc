...
func NewLesServer(etsc *etsc.etsc, config *etsc.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(etsc.BlockChain().Config(), light.DefaultServerIndexerConfig, false, config.NetworkId, etsc.EventMux(), etsc.Engine(), newPeerSet(), etsc.BlockChain(), etsc.TxPool(), etsc.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(etsc.BlockChain().Genesis().Hash(), pv)
	}

	srv := &LesServer{
		lesCommons: lesCommons{
			config:           config,
			chainDb:          etsc.ChainDb(),
			iConfig:          light.DefaultServerIndexerConfig,
			chtIndexer:       light.NewChtIndexer(etsc.ChainDb(), nil, params.CHTFrequencyServer, params.HelperTrieProcessConfirmations),
			bloomTrieIndexer: light.NewBloomTrieIndexer(etsc.ChainDb(), nil, params.BloomBitsBlocks, params.BloomTrieFrequency),
			protocolManager:  pm,
		},
		quitSync:  quitSync,
		lesTopics: lesTopics,
	...
	srv.chtIndexer.Start(etsc.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(etsc.ChainDb())
	return srv, nil
}
...
func newCostStats(db etscdb.Database) *requestCostStats {
