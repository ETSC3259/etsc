...
func (p *peer) Info() *etsc.PeerInfo {
	return &etsc.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
...
func (p *peer) RequestHelperTrieProofs(reqID, cost uint64, data interface{}) error {
	...
	case lpv1:
		...
		return sendRequest(p.rw, getsceaderProofsMsg, reqID, cost, reqs)
	case lpv2:
		...
		return sendRequest(p.rw, getscelperTrieProofsMsg, reqID, cost, reqs)
	default:
		panic(nil)
	}
}
