...
type ChainIndexer struct {
	chainDb  etscdb.Database      // Chain database to index the data from
	indexDb  etscdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb, indexDb etscdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
