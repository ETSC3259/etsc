...
// genTxRing returns a block generator that sends etsc in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db etscdb.Database
	if !disk {
		db = etscdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "etsc-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = etscdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, etschash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, etschash.NewFaker(), vm.Config{}, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db etscdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "etsc-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := etscdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, etschash.NewFaker(), vm.Config{}, nil)
		if err != nil {
			b.Fatalf("error creating chain: %v", err)
		}

		for n := uint64(0); n < count; n++ {
			header := chain.getsceaderByNumber(n)
			if full {
				hash := header.Hash()
