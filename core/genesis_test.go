...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(etscdb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db etscdb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, new(Genesis))
			},
			wantErr:    errGenesisNoConfig,
			wantConfig: params.AllEtschashProtocolChanges,
		},
		{
		...
		},
		{
			...
			fn: func(db etscdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, etschash.NewFullFaker(), vm.Config{}, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, etschash.NewFaker(), db, 4, nil)
