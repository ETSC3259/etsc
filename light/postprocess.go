...
import (
	...
	"github.com/ETSC3259/etsc/common/bitutil"
...
func GetChtRoot(db etscdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
// Note that sectionIdx is specified according to LES/1 CHT section size.
func StoreChtRoot(db etscdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    etscdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db etscdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := etscdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, etscdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func GetBloomTrieRoot(db etscdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db etscdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db etscdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := etscdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, etscdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
