...
func makeHeaderChain(parent *types.Header, n int, db etscdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), etschash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (etscdb.Database, *LightChain, error) {
	db := etscdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, etschash.NewFaker())
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, etschash.NewFullFaker())
	if err != nil {
		panic(err)
	}
	return lc
...
func testFork(t *testing.T, LightChain *LightChain, i, n int, comparator func(td1, td2 *big.Int)) {
	...
	hash1 = LightChain.getsceaderByNumber(uint64(i)).Hash()
	hash2 = LightChain2.getsceaderByNumber(uint64(i)).Hash()
	if hash1 != hash2 {
...
func (odr *dummyOdr) Database() etscdb.Database {
	return odr.db
}
...
func testReorg(t *testing.T, first, second []int, td int64) {
	...
	for header := bc.getsceaderByNumber(bc.CurrentHeader().Number.Uint64() - 1); header.Number.Uint64() != 0; prev, header = header, bc.getsceaderByNumber(header.Number.Uint64()-1) {
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, etschash.NewFaker())
	if err != nil {
