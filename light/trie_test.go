...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = etscdb.NewMemDatabase()
		lightdb = etscdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, etschash.NewFullFaker(), vm.Config{}, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, etschash.NewFaker(), fulldb, 4, testChainGen)
