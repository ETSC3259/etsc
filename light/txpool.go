...
// to the etsc network. The implementations of the functions should be non-blocking.
//
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	oldh := pool.chain.getsceaderByHash(pool.head)
	newh := newHeader
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = pool.chain.getsceader(oldh.ParentHash, oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHashes = append(newHashes, newh.Hash())
			newh = pool.chain.getsceader(newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	// block limit gas.
	header := pool.chain.getsceaderByHash(pool.head)
