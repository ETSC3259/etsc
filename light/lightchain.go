...
// available in the database. It initialises the default etsc header
// validator.
...
func (self *LightChain) loadLastState() error {
	if head := rawdb.ReadHeadHeaderHash(self.chainDb); head == (common.Hash{}) {
		...
		if header := self.getsceaderByHash(head); header != nil {
...
func (self *LightChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if head := self.hc.CurrentHeader(); head.Hash() == hash {
			self.hc.SetCurrentHeader(self.getsceader(head.ParentHash, head.Number.Uint64()-1))
		}
	}
}
...
// caching it if found.
func (self *LightChain) getsceader(hash common.Hash, number uint64) *types.Header {
	return self.hc.getsceader(hash, number)
}

// getsceaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (self *LightChain) getsceaderByHash(hash common.Hash) *types.Header {
	return self.hc.getsceaderByHash(hash)
}
...
func (self *LightChain) getsceaderByNumber(number uint64) *types.Header {
	return self.hc.getsceaderByNumber(number)
}

// getsceaderByNumberOdr retrieves a block header from the database or network
// by number, caching it (associated with its hash) if found.
func (self *LightChain) getsceaderByNumberOdr(ctx context.Context, number uint64) (*types.Header, error) {
	if header := self.hc.getsceaderByNumber(number); header != nil {
		return header, nil
	}
	return getsceaderByNumber(ctx, self.odr, number)
}
...
func (self *LightChain) SyncCht(ctx context.Context) bool {
	...
	if header, err := getsceaderByNumber(ctx, self.odr, latest); header != nil && err == nil {
		self.mu.Lock()
		defer self.mu.Unlock()
