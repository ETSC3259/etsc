...
// Tests that post etsc protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, etschash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
...
func testBroadcastBlock(t *testing.T, totalPeers, broadcastExpected int) {
	...
	chain, _ := core.GenerateChain(gspec.Config, genesis, etschash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {})
