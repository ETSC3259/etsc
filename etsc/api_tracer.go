...
import (
	...
	"github.com/ETSC3259/etsc/etsc/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.etsc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.etsc.blockchain.CurrentBlock()
	default:
		from = api.etsc.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.etsc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.etsc.blockchain.CurrentBlock()
	default:
		to = api.etsc.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabase(api.etsc.ChainDb())

	if number := start.NumberU64(); number > 0 {
		start = api.etsc.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					...
					vmctx := core.NewEVMContext(msg, task.block.Header(), api.etsc.blockchain, nil)
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.etsc.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.etsc.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.etsc.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		block = api.etsc.blockchain.CurrentBlock()
	default:
		block = api.etsc.blockchain.GetBlockByNumber(uint64(number))
	}
...
func (api *PrivateDebugAPI) TraceBlockByHash(ctx context.Context, hash common.Hash, config *TraceConfig) ([]*txTraceResult, error) {
	block := api.etsc.blockchain.GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) TraceBadBlock(ctx context.Context, index int, config *TraceConfig) ([]*txTraceResult, error) {
	if blocks := api.etsc.blockchain.BadBlocks(); index < len(blocks) {
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.etsc.engine.VerifyHeader(api.etsc.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.etsc.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				...
				vmctx := core.NewEVMContext(msg, block.Header(), api.etsc.blockchain, nil)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	for i := uint64(0); i < reexec; i++ {
		block = api.etsc.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.etsc.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.etsc.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &etscapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  etscapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	// Create the parent state database
	block := api.etsc.blockchain.GetBlockByHash(blockHash)
	if block == nil {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		context := core.NewEVMContext(msg, block.Header(), api.etsc.blockchain, nil)
		if idx == txIndex {
