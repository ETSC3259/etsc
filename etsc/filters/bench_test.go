...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/getsc/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := etscdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = etscdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db etscdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*etscdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db etscdb.Database) {
