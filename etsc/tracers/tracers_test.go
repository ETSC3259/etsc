...
import (
	...
	"github.com/ETSC3259/etsc/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a getsc console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = etsc.getBlock(etsc.getTransaction(tx).blockHash);
  var genesis = etsc.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.etsc.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  etsc.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(etscdb.NewMemDatabase(), test.Genesis.Alloc)
