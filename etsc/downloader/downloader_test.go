...
type downloadTester struct {
	...
	stateDb etscdb.Database // Database used by the tester for syncing from peers
	peerDb  etscdb.Database // Database of the peers containing all data
...
func newTester() *downloadTester {
	...
	tester.stateDb = etscdb.NewMemDatabase()
...
func (dl *downloadTester) HasHeader(hash common.Hash, number uint64) bool {
	return dl.getsceaderByHash(hash) != nil
}
...
// getsceader retrieves a header from the testers canonical chain.
func (dl *downloadTester) getsceaderByHash(hash common.Hash) *types.Header {
	dl.lock.RLock()
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", etsc.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", etsc.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", etsc.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", etsc.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want etsc.SyncProgress) {
	t.Helper()
	p := d.Progress()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", etsc.SyncProgress{
