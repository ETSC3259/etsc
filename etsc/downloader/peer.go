...
type peerConnection struct {
	...
	version int        // Etsc protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [etsc/62+] requested on etsc/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [etsc/62+] requested on etsc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [etsc/63+] requested on etsc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [etsc/63+] requested on etsc/%d", p.version))
	}
	// Short circuit if the peer is already fetching
