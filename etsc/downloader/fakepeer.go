...
func NewFakePeer(id string, db etscdb.Database, hc *core.HeaderChain, dl *Downloader) *FakePeer {
...
func (p *FakePeer) RequestHeadersByHash(hash common.Hash, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.getsceaderByHash(hash)
		if origin == nil {
			break
		}
		...
		if reverse {
			for i := 0; i <= skip; i++ {
				if header := p.hc.getsceader(hash, number); header != nil {
			...
			if header := p.hc.getsceaderByNumber(next); header != nil {
...
func (p *FakePeer) RequestHeadersByNumber(number uint64, amount int, skip int, reverse bool) error {
	...
	for !unknown && len(headers) < amount {
		origin := p.hc.getsceaderByNumber(number)
		if origin == nil {
			break
		}
		if reverse {
